...
const useQlikRemoveBookmark = () => {
    const [qAction, setQAction] = useState<any>({ loading: false, qResponse: null, error: null })
    const { qAppId } = useQlikApp()
    const { qAppMap } = useQlikAppContext()

    const setRemoveBookmark = useCallback(
        async (id: string, qlikAppId?: string) => {
            ...
            if (qApp?.qApi) {
                try {
                    ...
                    r = await qApp?.qApi?.$apiBookmark.remove(id)
    ...
    return { qAction, setRemoveBookmark }
}

export default useQlikRemoveBookmark
