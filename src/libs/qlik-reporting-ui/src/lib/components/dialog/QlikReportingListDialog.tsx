...
interface IQlikReportingListDialogProps {
    qlikAppId: string
    views: string[]
...
const QlikReportingListDialog: FC<IQlikReportingListDialogProps> = ({
    qlikAppId,
    color,
    views,
    LoaderComponent,
    ...
    const [isSharedChecked, setIsSharedChecked] = useState<boolean>(false)
    const [reports, setReports] = useState([])
    const { qAppMap } = useQlikAppContext()
    const { qMasterDimensionsMap, qMasterMeasuresMap } = useQlikMasterItemContext()
    const { appUser } = useAuthContext()
    const { reportDefaultFilters, loadUserReports, clearReport, deleteReport, patchReport } =
    ...
    const columns = [
        ...
        {
            id: 'shared',
            label: t(translation.reportingDialogTabListColumnIsShared),
            minWidth: 50,
            format: 'icon',
            visible: views.includes('Share')
        },
        {
            id: 'updatedAt',
    ...
    const loadData = useCallback(
        async (isAdminOn = false) => {
            try {
                ...
                const reports = await loadUserReports(isAdminOn, hasWriteAccess)

                const filteredRows = isAdminOn
                    ? reports.filter(report => !!report.isSystem)
                    : reports
                ...
                const rows = filteredRows.map(report => ({
                    ...
                    isShared: report.sharedWithOthers || isAdminOn,
    ...
    const handleFiltering = (text: string, isShared = false) => {
        const rowsList = []
        for (const report of reports) {
            rowsList.push({
                ...
                vizType: report.visualizationType,
                isSystem: report.isSystem,
                isShared: report.sharedWithOthers || isUserAdmin,
                appUserId: report.appUserId,
                updatedAt: formatDateHelper(report.updatedAt)
            })
        }
        ...
        if (isShared) filteredList = filteredList.filter(item => item.isShared)

        setRows(filteredList)
    }

    const handleIsSharedCheck = async (event: any) => {
        setIsLoading(true)
        setIsSharedChecked(event.target.checked)
        setPage(0)
    ...
    const handleSearchChange = async e => {
        handleFiltering(e.target.value, isSharedChecked)
        setText(e.target.value)
        setPage(0)
    }

    const handleCancelSearch = () => {
        handleFiltering('', isSharedChecked)
        setText('')
        setPage(0)
    }
                ...
                {!isUserAdmin && (
                    <Box className={classes.checkboxContainer}>
                        <Checkbox
                            ...
                            color="secondary"
                            onChange={handleIsSharedCheck}
                            disabled={isLoading || isUserAdmin}
                            disableRipple
                            size="medium"
                        />
                        <Typography className={classes.checkboxText}>
                            {t(translation.reportingDialogListIsShared)}
                        </Typography>
                    </Box>
                )}
            </Box>
                                ...
                                <TableCell colSpan={7}>
                                    <Box
                                        ...
                                        <Typography
                                            ...
                                            {t(translation.reportingDialogMyReportsEmpty)}
                                        </Typography>
                                    </Box>
                                </TableCell>
                            </TableRow>
...
export default QlikReportingListDialog
